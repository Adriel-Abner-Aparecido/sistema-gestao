...
export const CategoriaModal = (props) => {
    ...
    const salvar = () => {
        ...
        post(process.env.NEXT_PUBLIC_API_URL + '/categorias', data).then((res) => {
            if (res.mensagem) {
    ...
    const deletar = () => {
        ...
        remove(process.env.NEXT_PUBLIC_API_URL + '/categorias/' + idCor + '').then((res) => {
            if (res.mensagem) {
                if (res.mensagem == "falha na autenticação") {
                    console.log('falha na autenticação');
    ...
    return (
        <div id="medium-modal" className={props.statusModal + " pt-28 first-letter:overflow-y-auto overflow-x-hidden fixed z-50 w-full h-modal sm:h-auto sm:w-1/4"}>
            <div className="relative p-4 w-full h-full sm:h-auto">
                {/*<!-- Modal content --> */}
                <div className="relative rounded-lg shadow border bg-gray-200 text-gray-700">
                    {/*<!-- Modal header --> */}
                    <div className="flex justify-between items-center p-5 rounded-t border-b">
                        <h3 className="text-xl font-medium">
                            Categoria
                        </h3>
                        <button onClick={closeModal} type="button" className="text-gray-400 bg-transparent rounded-lg text-sm p-1.5 ml-auto inline-flex items-center hover:bg-gray-400 hover:text-white">
                    ...
                    <div className="p-6 space-y-6">
                        <div>
                            ...
                            <input id='nomeCategoria' className="border text-sm rounded-lg block w-full p-2.5" required />
                        </div>
                    </div>
                    {/*<!-- Modal footer --> */}
                    {loading ? (
