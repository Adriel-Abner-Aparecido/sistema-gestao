...
export const CategoriaContasModal = (props) => {
    const [loading, setLoading] = useState(false);

    const closeModal = () => {
        if (props.hiddenCategoriaContas == "") {
            props.setHiddenCategoriaContas("hidden");
        }
    }
    ...
    const salvar = () => {
        ...
        let nome = document.getElementById('nomeCategoriaContas').value;

        const data = {
            nome: nome
        }

        post(process.env.NEXT_PUBLIC_API_URL + '/categoriacontas', data).then((res) => {
            if (res.mensagem) {
                ...
                if (res.message == undefined) {
                    ...
                    closeModal();
                    document.getElementById('nomeCategoriaContas').value = '';
                    props.setAtualizarCategoriaContas(!props.atualizarCategoriaContas)
                } else {
    ...
    return (
        <div id="medium-modal" className={props.hiddenCategoriaContas + " top-1/4 first-letter:overflow-y-auto overflow-x-hidden fixed z-50 w-full h-modal md:h-auto sm:w-1/4"}>
                ...
                <div className="relative rounded-lg shadow bg-gray-200">
                    ...
                    <div className="p-6 space-y-6">
                        <div>
                            ...
                            <input id='nomeCategoriaContas' className="border text-sm rounded-lg block w-full p-2.5" required />
                        </div>
                    </div>
                    {/*<!-- Modal footer --> */}
                    {loading ? (
