...
export const CstIcmsModal = (props) => {
    ...
    const salvar = () => {
        ...
        let descricao = document.getElementById('nomeCstIcms').value;
        let cstIcms = document.getElementById('numeroCstIcms').value;
        ...
        post(process.env.NEXT_PUBLIC_API_URL + '/csticms', data).then((res) => {
            if (res.mensagem) {
    ...
    const deletar = () => {
        setLoading(true);
        let idCor = props.tipoProdutoSelecionado.id

        remove(process.env.NEXT_PUBLIC_API_URL + '/tipoprodutos/' + idCor + '').then((res) => {
            if (res.mensagem) {
                if (res.mensagem == "falha na autenticação") {
                    console.log('falha na autenticação');
            ...
            <div className="relative p-4 w-full h-full sm:h-auto">
                ...
                <div className="relative rounded-lg shadow bg-gray-600">
                    ...
                    <div className="p-6 space-y-6">
                        <div>
                            <label className="block mb-2 text-sm font-medium text-gray-300">Cst Icms</label>
                            <input id='numeroCstIcms' className="border text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 bg-gray-500 border-gray-400 placeholder-gray-300 text-white" required />
                        </div>
                        <div>
                            <label className="block mb-2 text-sm font-medium text-gray-300">Descrição</label>
                            <input id='nomeCstIcms' className="border text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 bg-gray-500 border-gray-400 placeholder-gray-300 text-white" required />
                        </div>
                    </div>
                    {/*<!-- Modal footer --> */}
                    {loading ? (
