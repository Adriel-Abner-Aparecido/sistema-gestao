...
export const SubcategoriaModal = (props) => {
    ...
    useEffect(() => {
        if (props.subcategoriaSelecionada) {
    ...
    const salvar = () => {
        ...
        post(process.env.NEXT_PUBLIC_API_URL + '/subcategorias', data).then((res) => {
            if (res.mensagem) {
    ...
    const deletar = () => {
        ...
        remove(process.env.NEXT_PUBLIC_API_URL + '/subcategorias/' + idCor + '').then((res) => {
            if (res.mensagem) {
                if (res.mensagem == "falha na autenticação") {
                    console.log('falha na autenticação');
                ...
                <div className="relative rounded-lg shadow bg-gray-200">
                    ...
                    <div className="p-6 space-y-6">
                        <div>
                            ...
                            <input id='nomeSubcategoria' className="border text-sm rounded-lg block w-full p-2.5" required />
                        </div>
                    </div>
                    {/*<!-- Modal footer --> */}
                    {loading ? (
