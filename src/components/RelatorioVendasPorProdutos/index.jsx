...
export const RelatorioVendasPorProdutos = () => {
    ...
    useEffect(() => {
        if (listVendasPorProdutos.length != 0) {
            let totalQuantidade = listVendasPorProdutos.reduce((total, item) => total + item.quantidade_vendida, 0);
            let totalValor = listVendasPorProdutos.reduce((total, item) => total + (item.valor_total || 0), 0);
            setTotalQuantidade(totalQuantidade);
            setTotalValor(totalValor);
        }
    }, [listVendasPorProdutos])
    ...
    const gerarRelatorio = () => {
        ...
        if (dataInicio != "" && dataFinal != "") {
            ...
            if (checkDates(dataInicio, dataFinal)) {
                get(process.env.NEXT_PUBLIC_API_URL + '/vendasporproduto/periodo/' + dataInicio + '/' + dataFinal + '').then((res) => {
                    if (res.mensagem) {
                ...
                get(process.env.NEXT_PUBLIC_API_URL + '/vendasporproduto/periodo/' + dataInicio + '/' + dataFinal + '').then((res) => {
                    if (res.mensagem) {
                        ...
                        if (res.length != 0) {
                            ...
                            XLSX.writeFile(wb, "vendas_por_produtos_" + formatDate(dataAgora) + ".xlsx");
                        }
                    }
                })
            } else {
                    ...
                    <tbody>
                        {listVendasPorProdutos.map((data, index) => (
                            <tr key={index} className={index % 2 ? 'bg-gray-100' : ''}>
                                <td className="border px-4 py-2">{data.produto}</td>
