...
export const ColecaoModal = (props) => {
    ...
    const salvar = () => {
        ...
        post(process.env.NEXT_PUBLIC_API_URL + '/colecoes', data).then((res) => {
            if (res.mensagem) {
    ...
    const deletar = () => {
        ...
        remove(process.env.NEXT_PUBLIC_API_URL + '/colecoes/' + idCor + '').then((res) => {
            if (res.mensagem) {
                if (res.mensagem == "falha na autenticação") {
                    console.log('falha na autenticação');
    ...
    return (
        <div id="medium-modal" className={props.statusModal + " pt-28 first-letter:overflow-y-auto overflow-x-hidden fixed z-50 w-full h-modal md:h-auto sm:w-1/4"}>
                ...
                <div className="relative rounded-lg shadow bg-gray-200">
                    ...
                    <div className="p-6 space-y-6">
                        <div>
                            ...
                            <input id='nomeColecao' className="border text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder='Ex.: Inverno max. 45 caracter' required />
                        </div>
                    </div>
                    {/*<!-- Modal footer --> */}
                    {loading ? (
