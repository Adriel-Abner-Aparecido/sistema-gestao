...
export const RelatorioLucroBrutoPorProduto = () => {
    const [hiddenTable, setHiddenTable] = useState('hidden');
    const [listVendasPorProdutos, setListVendasPorProdutos] = useState([]);
    const [totalQuantidade, setTotalQuantidade] = useState(0);
    const [totalValor, setTotalValor] = useState(0);
    const [custoTotal, setCustoTotal] = useState(0)
    const [totalProdutoVendido, setTotalProdutoVendido] = useState(0)
            ...
            if (checkDates(dataInicio, dataFinal)) {
                get(process.env.NEXT_PUBLIC_API_URL + '/lucrobrutoporproduto/periodo/' + dataInicio + '/' + dataFinal + '').then((res) => {
                    if (res.mensagem) {
                        ...
                        setListVendasPorProdutos(res)
                        let totalQuantidade = res.reduce((total, item) => total + item.total_vendas, 0)
                        let totalValor = res.reduce((total, item) => total + (item.venda_total || 0), 0)
                        let totalCusto = res.reduce((total, item) => total + (item.custo_total || 0), 0)
                        let totalProdutoVendido = res.reduce((total, item) => total + (item.quantidade_produto_vendida || 0), 0)
                        let totalDesconto = res.reduce((total, item) => total + (item.desconto + ((item.valor_unitario * item.quantidade_produto_vendida) - item.venda_total)), 0)
                        const descontoTotalVenda = res.reduce((total, item) => total + (item.desconto), 0)
                        setTotalQuantidade(totalQuantidade)
                        setTotalValor(totalValor)
                        setCustoTotal(totalCusto)
                        setTotalProdutoVendido(totalProdutoVendido)
                        setTotalDesconto(totalDesconto)
                        setLucroTotal(totalValor - descontoTotalVenda - totalCusto)
                        setHiddenTable("")
                    }
                })
            } else {
                    ...
                    if (res.mensagem) {
                        ...
                        if (res.length != 0) {
                            ...
                            const vendasParaExcel = res.map(venda => ({
                                ...
                                Quantidade_Vendida: Number.isInteger(venda.quantidade_produto_vendida) ? venda.quantidade_produto_vendida : (venda.quantidade_produto_vendida || 0).toFixed(2),
                                Valor: (venda.venda_total || 0).toFixed(2).replace('.', ','),
                                Custo: (venda.custo_total || 0).toFixed(2).replace('.', ','),
                                Desconto: (venda.desconto || 0).toFixed(2).replace('.', ','),
                                Lucro: (venda.venda_total - venda.custo_total || 0).toFixed(2).replace('.', ',')
                            }));

                            const quantidadeTotal = res.reduce((total, item) => total + item.total_vendas, 0)
                            const valorTotal = res.reduce((total, item) => total + item.venda_total, 0)
                            const totalCusto = res.reduce((total, item) => total + item.custo_total, 0)
                            const qtdProdutoVendida = res.reduce((total, item) => total + item.quantidade_produto_vendida, 0)

                            vendasParaExcel.push({
                                Produto: "TOTAL",
                                Vendas: quantidadeTotal,
                                Quantidade_Vendida: Number.isInteger(qtdProdutoVendida) ? qtdProdutoVendida : qtdProdutoVendida.toFixed(2),
                                Valor: (valorTotal || 0).toFixed(2).replace('.', ','),
                                Desconto: (totalDesconto || 0).toFixed(2).replace('.', ','),
                                Custo: (totalCusto || 0).toFixed(2).replace('.', ','),
                                Lucro: (lucroTotal || 0).toFixed(2).replace('.', ',')
                            })
                ...
                <table className="w-full text-left border-collapse table-auto">
                    <thead>
                        <tr className="bg-gray-200">
                            ...
                            <th className="border px-4 py-2">Quantidade Vendida</th>
                            <th className="border px-4 py-2">Valor Vendido</th>
                            <th className="border px-4 py-2">Desconto</th>
                            <th className='border px-4 py-2'>Lucro Bruto</th>
                        </tr>
                    </thead>
                    <tbody>
                        {listVendasPorProdutos.map((data, index) => (
                            <tr key={index} className={index % 2 ? 'bg-gray-100' : ''}>
                                <td className="border px-4 py-2">{data.produto_id}</td>
                                <td className="border px-4 py-2">{data.nome}</td>
                                <td className="border px-4 py-2">{data.total_vendas}</td>
                                <td className="border px-4 py-2">{Number.isInteger(data.quantidade_produto_vendida) ? data.quantidade_produto_vendida : (data.quantidade_produto_vendida || 0).toFixed(2)}</td>
                                <td className="border px-4 py-2">{Brl(data.venda_total || 0)}</td>
                                <td className="border px-4 py-2">{Brl(data.desconto + ((data.valor_unitario * data.quantidade_produto_vendida) - data.venda_total))}</td>
                                <td className="border px-4 py-2">{Brl((data.venda_total - data.custo_total) || 0)}</td>
                            </tr>
                        ))}
                    </tbody>
                    <tfoot>
                        <tr className="bg-gray-200">
                            ...
                            <th className="border px-4 py-2"></th>
                            <th className="border px-4 py-2">{Number.isInteger(totalProdutoVendido) ? totalProdutoVendido : (totalProdutoVendido || 0).toFixed(2)}</th>
                            <th className="border px-4 py-2">{Brl(totalValor)}</th>
                            <th className="border px-4 py-2">{Brl(totalDesconto)}</th>
                            <th className="border px-4 py-2">{Brl(lucroTotal)}</th>
                        </tr>
                    </tfoot>
                </table>
            </div>
